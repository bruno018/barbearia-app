...
function buildMonthSelector() {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    opts.push(`<option value="${value}"${i === 0 ? ' selected' : ''}>${label.charAt(0).toUpperCase() + label.slice(1)}</option>`);
  }
...
async function downloadMonthlyPDF() {
  ...
  const monthLabel = new Date(selYear, selMonthIdx, 1).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' });
  ...
  if (!monthly.length) {
    doc.setTextColor(160, 140, 120); doc.setFontSize(11); doc.setFont('helvetica', 'italic');
  ...
  doc.setDrawColor(196, 154, 108); doc.setLineWidth(0.3);
  doc.roundedRect(margin, y, W - margin * 2, 40, 3, 3, 'S');
  doc.setFontSize(11); doc.setFont('helvetica', 'bold'); doc.setTextColor(60, 30, 15);
  doc.text('Resumo do Mes', margin + 5, y + 10);
  doc.setFontSize(9); doc.setFont('helvetica', 'normal');
  doc.setTextColor(30, 100, 60);  doc.text('Confirmados: ' + confirmed.length, margin + 5, y + 20);
  doc.setTextColor(160, 100, 20); doc.text('Pendentes: '   + pending.length,   margin + 65, y + 20);
  doc.setTextColor(160, 30, 30);  doc.text('Cancelados: '  + cancelled.length, margin + 120, y + 20);
  doc.setTextColor(107, 58, 42); doc.setFontSize(10); doc.setFont('helvetica', 'bold');
  ...
  doc.setTextColor(160, 30, 30); doc.setFontSize(9); doc.setFont('helvetica', 'normal');
  doc.text('Receita perdida (cancelados): R$ ' + lostRev, margin + 95, y + 38);
  y += 50;

  doc.setFontSize(11); doc.setFont('helvetica', 'bold'); doc.setTextColor(60, 30, 15);
  doc.text('Por Servico', margin, y); y += 6;
  doc.setFillColor(107, 58, 42); doc.rect(margin, y, W - margin * 2, 8, 'F');
  doc.setTextColor(245, 236, 215); doc.setFontSize(8.5); doc.setFont('helvetica', 'bold');
  doc.text('Servico', margin+3, y+5.5); doc.text('Confirmados', margin+62, y+5.5);
  doc.text('Cancelados', margin+100, y+5.5); doc.text('Pendentes', margin+135, y+5.5);
  doc.text('Receita', margin+158, y+5.5); y += 8;

  Object.entries(svcCount).forEach(([name, data], i) => {
    doc.setFillColor(...(i%2===0 ? [255,252,248] : [245,240,235]));
    doc.rect(margin, y, W-margin*2, 8, 'F');
    doc.setTextColor(40,20,10); doc.setFontSize(8.5); doc.setFont('helvetica','normal');
    doc.text(name, margin+3, y+5.5);
    doc.setTextColor(30,100,60);  doc.text(String(data.confirmed||0), margin+72, y+5.5);
    doc.setTextColor(160,30,30);  doc.text(String(data.cancelled||0), margin+110, y+5.5);
    doc.setTextColor(160,100,20); doc.text(String(data.pending||0),   margin+145, y+5.5);
    doc.setTextColor(107,58,42);  doc.setFont('helvetica','bold');
    doc.text('R$ '+(data.revenue||0), margin+158, y+5.5); y += 8;
  });

  y += 10;
  doc.setFontSize(11); doc.setFont('helvetica','bold'); doc.setTextColor(60,30,15);
  doc.text('Todos os Agendamentos', margin, y); y += 6;
  doc.setFillColor(107,58,42); doc.rect(margin, y, W-margin*2, 8, 'F');
  doc.setTextColor(245,236,215); doc.setFontSize(8); doc.setFont('helvetica','bold');
  doc.text('Data',margin+3,y+5.5); doc.text('Horario',margin+30,y+5.5);
  doc.text('Cliente',margin+50,y+5.5); doc.text('Servico',margin+100,y+5.5);
  doc.text('Valor',margin+138,y+5.5); doc.text('Status',margin+158,y+5.5); y += 8;
  ...
  [...monthly].sort((a,b) => {
    ...
    doc.setFontSize(8); doc.setFont('helvetica','normal'); doc.setTextColor(40,20,10);
    doc.text(new Date(b.date_key).toLocaleDateString('pt-BR'), margin+3, y+5);
    doc.text(b.slot, margin+30, y+5);
    doc.text(b.client_name.length>18 ? b.client_name.substring(0,16)+'..' : b.client_name, margin+50, y+5);
    doc.text(b.service.length>13 ? b.service.substring(0,11)+'..' : b.service, margin+100, y+5);
  ...
  y += 4;
  doc.setDrawColor(196,154,108); doc.setLineWidth(0.4); doc.line(margin,y,W-margin,y); y += 7;
  doc.setFontSize(10); doc.setFont('helvetica','bold'); doc.setTextColor(30,100,60);
  doc.text('Receita total confirmada: R$ '+revenue, margin, y);
  doc.setTextColor(160,30,30); doc.setFontSize(9); doc.setFont('helvetica','normal');
  doc.text('Receita perdida com cancelamentos: R$ '+lostRev, margin, y+7);

  doc.save(`relatorio-${sel.value}.pdf`);
  showToast('📄 PDF gerado!', `relatorio-${sel.value}.pdf`);
}

// Polling — só roda após login
setInterval(() => {
